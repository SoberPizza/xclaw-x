...
**截屏（眼睛）→ YOLO + RapidOCR + ConvNeXt-Tiny 感知（视觉皮层）→ 结构化 JSON（语言区）→ OS 原生键鼠操作（手）**
...
│   │   ├── pipeline_backend.py # 默认后端：YOLO + OCR + ConvNeXt-Tiny
│   │   ├── omniparser.py  # OmniDetector（YOLO 双后端：ONNX / ultralytics）
│   │   ├── icon_classifier.py # ConvNeXt-Tiny 图标形状分类（stub）
...
| 图标分类 | ConvNeXt-Tiny CUDA（stub） | ConvNeXt-Tiny CPU（stub） |
...
- `PerceptionEngine`（`perception/engine.py`）是感知层单例，委托 `PerceptionBackend` 协议：
  - 默认后端 `PipelineBackend`（`pipeline_backend.py`）组合三个子模块：
    ...
    - `IconClassifier`：ConvNeXt-Tiny 图标形状分类（stub，模型尚未训好）
...
- `xclaw click X Y [--double]` — 点击 + 自动感知
- `xclaw type TEXT` — 打字 + 自动感知
- `xclaw press KEY` — 按键 + 自动感知
- `xclaw scroll up|down AMOUNT` — 滚动 + 自动感知
...
- ConvNeXt-Tiny 分类器：`models/icon_classifier/model.pt`（尚未训好，接口已就绪）
