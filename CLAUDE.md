...
- Windows：`uv sync --extra win`（PyTorch 从 `download.pytorch.org/whl/cu121` 安装，PaddlePaddle GPU 从 `paddlepaddle.org.cn/packages/stable/cu126/` 安装）
...
| OCR | PaddleOCR GPU (PaddlePaddle 3.x cu126) | PaddleOCR CPU + MKL-DNN (PaddlePaddle 3.x) |
| MiniCPM-V | CUDA FP16 | CPU FP32（MPS 有 gather bug） |
| 守护进程 IPC | Named Pipe | Unix Domain Socket |
| torch 索引 | `pytorch-cu121` | PyPI 默认（MPS） |
| PaddlePaddle 索引 | `paddlepaddle.org.cn/packages/stable/cu126/` | PyPI 默认 |
...
项目通过 `[tool.uv.index]` + `[tool.uv.sources]` 配置了两个自定义索引：

- **pytorch-cu121** — Windows/Linux 的 PyTorch CUDA 12.1 wheels（macOS 走 PyPI 默认的 MPS 版本）
- **paddle-cu126** — Windows 的 PaddlePaddle GPU CUDA 12.6 wheels（PyPI 上的 `paddlepaddle-gpu` 最高仅 2.6.2，在 Python 3.12 下崩溃；3.x 版本只发布在 PaddlePaddle 官方索引）
